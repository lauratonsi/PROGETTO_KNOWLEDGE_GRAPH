...
&lt;https://w3id.org/bologna/resource/person/...&gt;
  ...
  <div style="text-align:center; margin:2rem 0">
    <a class="btn-webvowl"
       href="https://service.tib.eu/webvowl/#iri=https://lauratonsi.github.io/PROGETTO_KNOWLEDGE_GRAPH/schema.ttl"
...
<div class="section">
  ...
  Il file <code>bologna_KG_corretto.ttl</code> (16.244 triple) viene interrogato in tempo reale
  tramite <a href="https://comunica.dev/" target="_blank">Comunica</a>, un motore SPARQL&nbsp;1.1 open source.</p>
  <p style="font-size:0.88rem;color:var(--text-muted)"><strong>Nota:</strong>
  al primo avvio il browser scarica il motore (~3&nbsp;MB) e il file RDF (~1&nbsp;MB).
  Le query successive sono molto più rapide grazie alla cache del browser.</p>
...
<div class="section">
  ...
  <h4>Risultati completi (13 righe totali):</h4>
  <div class="table-wrap"><table>
    <tr><th>?tipo</th><th>?nomeVia</th><th>?nomePersona</th></tr>
    <tr><td>Fratelli</td><td>PASSAGGIO FRATELLI MARINCOLA</td><td>FRATELLI MARINCOLA</td></tr>
    <tr><td>Fratelli</td><td>VIA FRATELLI BORDONI</td><td>FRATELLI BORDONI</td></tr>
    <tr><td>Fratelli</td><td>VIA FRATELLI CARPIGIANI</td><td>FRATELLI CARPIGIANI</td></tr>
    ...
    <tr><td>Fratelli</td><td>VIA FRATELLI GRUPPI</td><td>FRATELLI GRUPPI</td></tr>
    <tr><td>Fratelli</td><td>VIA FRATELLI MUSI</td><td>FRATELLI MUSI</td></tr>
    <tr><td>Fratelli</td><td>VIA FRATELLI PINARDI</td><td>FRATELLI PINARDI</td></tr>
    <tr><td>Fratelli</td><td>VIA FRATELLI ROSSELLI</td><td>FRATELLI ROSSELLI</td></tr>
    <tr><td>Maria</td><td>ROTONDA MARIA ADELE CROCIONI MICHELINI</td><td>MARIA ADELE CROCIONI MICHELINI</td></tr>
    <tr><td>Maria</td><td>VIA MARIA CALLAS</td><td>MARIA CALLAS</td></tr>
    <tr><td>Maria</td><td>VIA MARIA GAETANA AGNESI</td><td>MARIA GAETANA AGNESI</td></tr>
    <tr><td>Maria</td><td>VIA MARIA MELATO</td><td>MARIA MELATO</td></tr>
  </table></div>
  <p style="font-size:0.88rem;color:var(--text-muted)">9 strade con "Fratelli" (tutte classificate Male) e 4 donne con "Maria" nel nome.
  Il caso del Passaggio Fratelli Marincola è emblematico: Isabella Marincola, staffetta partigiana afrodiscendente, è resa invisibile dal sostantivo maschile plurale.
  BIND assegna l'etichetta <em>?tipo</em> a ciascun ramo dell'UNION, rendendo i risultati combinati immediatamente leggibili.</p>
...
let _sparqlEng = null;
function _loadComunica() {{
  if (_sparqlEng) return Promise.resolve(_sparqlEng);
  const st = document.getElementById('sparql-status');
  st.className = 'sparql-status'; st.textContent = '⏳ Caricamento motore SPARQL…';
  return new Promise(function(ok, fail) {{
    const s = document.createElement('script');
    s.src = 'https://comunica.github.io/comunica-browser/versions/latest/comunica-browser.js';
    s.onload = function() {{
      _sparqlEng = new Comunica.QueryEngine();
      ok(_sparqlEng);
    }};
    s.onerror = function() {{ fail(new Error('Impossibile caricare il motore Comunica')); }};
...
async function runSparql() {{
  ...
  try {{
    const engine = await _loadComunica();
    const ttlUrl = new URL('bologna_KG_corretto.ttl', window.location.href).href;
    st.className = 'sparql-status'; st.textContent = '⏳ Interrogazione KG in corso…';
    const stream = await engine.queryBindings(sparql, {{ sources: [ttlUrl] }});
    const bindings = await stream.toArray();
    if (!bindings.length) {{ st.textContent = 'Nessun risultato trovato.'; return; }}
    const MAX = 200;
    const shown = bindings.slice(0, MAX);
    const vars = [];
    shown[0].keys().forEach(function(k) {{ vars.push(typeof k === 'string' ? k : k.value); }});
    ...
    shown.forEach(function(b) {{
      ...
      vars.forEach(function(v) {{
        const t = b.get(v);
        const val = t ? (t.value !== undefined ? t.value : String(t)) : null;
        h += '<td>' + (val !== null ? val.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;') : '<em>—</em>') + '</td>';
      }});
      h += '</tr>';
    }});
    h += '</table></div>';
    res.innerHTML = h;
    st.textContent = '✓ ' + bindings.length + ' risultat' + (bindings.length === 1 ? 'o' : 'i') +
      (bindings.length > MAX ? ' (mostrati i primi ' + MAX + ')' : '');
  }} catch(e) {{
